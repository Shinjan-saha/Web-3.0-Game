...
// Add Hammer.js gesture recognition
const playerElem = document.querySelector("[data-player]");
const hammer = new Hammer(playerElem);

hammer.get("swipe").set({ direction: Hammer.DIRECTION_VERTICAL });

hammer.on("swipeup", function (event) {
  makePlayerFly(); // Call the function to make the player fly when there is a swipe up gesture
});
...
function checkIfWeGotNft() {
  const playerRect = getPlayerRect();
  if (
    getNftRects().some((rect) => isCollision(rect, playerRect))
  ) {
...
function updateScore(delta) {
  score += delta * 0.01;
  scoreElem.textContent = `Player score: ${Math.floor(score)}`;
}
...
function handleLose() {
  ...
  gweiTotalScoreEleme.textContent = `Player total score ${window.totalGweiScore}`;
